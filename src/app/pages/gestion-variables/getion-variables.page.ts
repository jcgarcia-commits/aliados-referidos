...
import { UpsertVariable } from '../../components/upsert-variable/upsert-variable';
...
@Component({
  selector: 'app-test-page',
    imports: [LucideAngularModule, FormsModule,RouterLink, CommonModule, UpsertVariable],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  templateUrl: './gestion-variables.page.html',
...
export default class GestionSolicitudesPageComponent {
 ...
 readonly SquarePen = SquarePen;
  ...
  openModal(nuevo: boolean = true, empresa: IEmpresa = emptyEmpresa) {
    ...
    this.modal.set({ titulo: nuevo ? 'Crear Variable' : 'Actualizar Variable', visible: true });    
  }
...
toggleEstado(empresa: IEmpresa, value: boolean) {
  const updatedList = this.empresasList().map(e =>
    e.empresaId === empresa.empresaId ? { ...e, estado: value } : e
  );
  this.empresasList.set(updatedList);
}

toggleActivo(empresa: IEmpresa, value: boolean) {
  const updatedList = this.empresasList().map(e =>
    e.empresaId === empresa.empresaId ? { ...e, activo: value } : e
  );
  this.empresasList.set(updatedList);
}
